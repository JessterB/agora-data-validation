...
source_synId <- "syn12540368" # harmonized_targets_20220707.csv 
...
```{r echo=FALSE}
source("~/repos/rstudio/agora-data-validation/src/utils.R")
compare_subobjects(get(old_name), get(new_name), "nominatedtarget")
```
...
***
* !is.na(data_synapseid)/field_length(data_synapseid, min = 3, max = 25): 49 fails -> NA values are properly handled by the app
* !is.na(data_used_to_support_target_selection)/field_length(data_used_to_support_target_selection, min = 15, max = 2000) : 2 fails -> NA values TODO AG-365
* !is.na(study)/field_length(study, min = 2, max = 200): 7 fails -> NA values - see https://sagebionetworks.jira.com/browse/AG-846
* !is.na(validation_study_details)/field_length(validation_study_details, min = 0, max = 1000): 130 fails -> NA values - see https://sagebionetworks.jira.com/browse/AG-846
...
## AG-533 Missing/optional NT fields (see )
### data_synapseid
