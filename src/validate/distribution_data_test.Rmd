...
NOTE: Agora 3.0.x is pinned to Live v7 for now; any newer versions of this file will pick up the new 5pt scoring system, and should be released only as part of AG-656. Processing into Agora Live Data has been disabled via config.yaml for now, so manual manifest updates should not be required.
...
old_synId <-  "syn26274945.7" # m46 v7 Live (derived from v9 source) - pin to v7 until we are ready for new scores
...
source("~/repos/rstudio/agora-data-validation/src/rules/rules_distribution_data_lit.R")
...
GeneticsScores <- distribution_data_new[, "GeneticsScore"]
OmicsScores <- distribution_data_new[, "OmicsScore"]
LitScores <- distribution_data_new[, "LiteratureScore"]
OverallScores <- distribution_data_new[, "Logsdon"]
...
# LiteratureScore.distribution - 0 >= -1e-08: 1 fail (Testing v15)
```{r echo=FALSE}
# yields a dataframe containing the failure cases for a specific rule
v <- validator(LiteratureScore.distribution - 0 >= -1e-08)
cf <- confront(distribution_data_new, v)
out <- values(cf)
is_unique_fails <- distribution_data_new[!out,]
is_unique_fails
```

# Logsdon.bins.1 - 0 >= -1e-08: 1 fail (Testing v15)
```{r echo=FALSE}
# yields a dataframe containing the failure cases for a specific rule
v <- validator(Logsdon.bins.1 - 0 >= -1e-08)
cf <- confront(distribution_data_new, v)
out <- values(cf)
is_unique_fails <- distribution_data_new[!out,]
is_unique_fails
...
# Logsdon.bins.2 - 0 >= -1e-08: 1 fail (Testing v15)
```{r echo=FALSE}
# yields a dataframe containing the failure cases for a specific rule
v <- validator(Logsdon.bins.2 - 0 >= -1e-08)
cf <- confront(distribution_data_new, v)
out <- values(cf)
is_unique_fails <- distribution_data_new[!out,]
is_unique_fails
...
```{r}
source("~/repos/rstudio/agora-data-validation/src/utils.R")
source <- download_file(old_source_synId, type = "table")
...
cat("\nOverall total genes expected: ", "24659") # from overall_scores_test.json

cat("\n\nGenomics distribution total genes: ", sum(distribution_data_new$OmicsScore.distribution))
cat("\nGenomics total genes expected: ", "23876") # from overall_scores_test.json

cat("\n\nLiterature distribution total genes: ", sum(distribution_data_new$LiteratureScore.distribution))
cat("\nLiterature total genes expected: ", "15656") # from overall_scores_test.json

cat("\n\nGenetics distribution total genes: ", sum(distribution_data_new$GeneticsScore.distribution))
cat("\nGenetics total genes expected: ", "23319") # from overall_scores_test.json
```
