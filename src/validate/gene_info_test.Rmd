...
target_list_source_synId <- "syn12540368.47" #harmonized_targets_20231219.csv
...
```{r}
# TER source
exp_val <- download_file(ter_source_synId, type="csv")
assign(paste("ter", "source", sep="_"), exp_val, envir = .GlobalEnv)
```
...
22051 - NA values (-> 19309 v233)
* !is.na(name)/field_length(name, min = 0, max = 200): 
22052 - NA values (-> 19310 v233)
* !is.na(nominations)/nominations > 0/lengths(nominatedtarget) > 0: 
70373 fails (only expect nominated targets to pass this one) (-> 65866 v233)
* !is.na(summary): 
47402 fails - NA values  (-> 43107 v233)
...
```{r}
# yields a dataframe containing the failure cases for a specific rule
noms <- get_subobject(gene_info_new, "target_nominations")
v <- validator(field_length(study, min = 2, max = 200))
cf <- confront(noms, v)
out <- values(cf)
is_unique_fails <- noms[!out,]
is_unique_fails
```
...
## field_length(ensembl_release, min = 1, max = 3) -> 7 nna

```{r}
# yields a dataframe containing the failure cases for a specific rule
subobj <- get_subobject(gene_info_new, "ensembl_info")
v <- validator(field_length(ensembl_release, min = 1, max = 3))
cf <- confront(subobj, v)
out <- values(cf)
subobj$validation_result <- out
print(subobj)
```
...
target_list_source_new <- download_file("syn12540368.47", "target_list_source_new", type="csv")
...
## AG-1333 -  unarchived ensembl releases - is ensembl_release value correct or also rolled back to last archived?
```{r}
# gereate a map of ensembl release to ensembl archive; uses NAs for unavailable archives
releases <- c(54:111)
archives <- c("may2009", rep(NA, 20), "feb2014", rep(NA, 1), "oct2014", rep(NA, 2), "may2015", rep(NA, 14), "jan2019",
              "apr2019", "jul2019", "sept2019", "jan2020", "apr2020", "aug2020", "nov2020", "feb2021", "may2021", 
              "dec2021", "apr2022", "jul2022", "oct2022", "feb2023", "jul2023", "jan2024")
ensembl_map <- data.frame(ensembl_release = releases, ensembl_archive = archives)
ensembl_map
```

```{r}
# extract ensembl_infos
infos <- do.call(rbind.data.frame, lapply(gene_info_new$ensembl_info, as.data.frame, stringsAsFactors = FALSE))
keeps <- c("ensembl_release", "ensembl_permalink")
infos <- subset(infos, select = keeps)
infos
```

```{r}
# filter out the rows that are in release 110
historic_release_infos <- infos[!grepl("jul2023", infos$ensembl_permalink, fixed = TRUE),]
historic_release_infos
```
How many ENSGs map to unarchived ensembl releases?
```{r}
unarchived <- ensembl_map[is.na(ensembl_map$ensembl_archive),]
unarchived_releases <- unarchived$ensembl_release
unarchived_releases

impacted <- historic_release_infos[historic_release_infos$ensembl_release %in% unarchived_releases,]
impacted
```
...
gene_source
cat("\n***ensembl_release***")
cat("\nunique values: ", sort(unique(gene_source$ensembl_release)))
...
```
What gene(s) are the 'missing' PRs associated with?

PRs missing in Agora:  
* ENSG00000289022 
  -> ENSG00000263464 (104, 6 other PRs: ENSG00000289549, ENSG00000288867, ENSG00000289022, ENSG00000289240, ENSG00000288733, ENSG00000289542)
  -> ENSG00000256374 (104, 6 other PRs: ENSG00000288867, ENSG00000288733, ENSG00000289549, ENSG00000289022, ENSG00000289240, ENSG00000289542)
* ENSG00000289240 - same as above
* ENSG00000288733 - same as above
* ENSG00000289542 - same as above

The 2 PRs that aren't missing from Agora are:
ENSG00000289549 - PPIAL4D
ENSG00000288867 - PPIAL4C

```{r}

flat_pr[flat_pr$possible_replacement == "ENSG00000289022",]
flat_pr[flat_pr$possible_replacement == "ENSG00000289240",]
flat_pr[flat_pr$possible_replacement == "ENSG00000288733",]
flat_pr[flat_pr$possible_replacement == "ENSG00000289542",]
...
## AG-1250 - igap HASG filtering + ensembl release 110


```{r}
# json files
old_synId <-  "syn12548902.49" 
new_synId <- "syn17015359.222" 

source("~/repos/rstudio/agora-data-validation/src/utils.R")
download_files(old_synId, new_synId, "gene_info_pinned")
```

```{r}
old_ensgs <- gene_info_pinned_old$ensembl_gene_id
new_ensgs <- gene_info_pinned_new$ensembl_gene_id
removed_ensgs <- setdiff(old_ensgs, new_ensgs)
added_ensgs <- setdiff(new_ensgs, old_ensgs)

gene_info_added <- gene_info_pinned_old[gene_info_pinned_old$ensembl_gene_id %in% removed_ensgs,]
gene_info_removed <- gene_info_pinned_new[gene_info_pinned_new$ensembl_gene_id %in% added_ensgs,]

gene_info_added
gene_info_removed
```

do SRM results intersect with the added/removed ENSGs? (AG-1245)
```{r}
srm_ensgs <- proteomics_srm_new$ensembl_gene_id

removed_srm <- intersect(srm_ensgs, removed_ensgs)
added_srm <- intersect(srm_ensgs, added_ensgs)
...
AG-1245 SRM deduplication

```{r}
iptk <- c("ENSG00000100605", "ENSG00000274958")
gene_info_new[gene_info_new$ensembl_gene_id %in% iptk,]

ldha <- c("ENSG00000134333", "ENSG00000288299")
gene_info_new[gene_info_new$ensembl_gene_id %in% ldha,]

mapt <- c("ENSG00000186868", "ENSG00000277956", "ENSG00000276155")
gene_info_new[gene_info_new$ensembl_gene_id %in% mapt,]

ndufa6 <- c("ENSG00000184983", "ENSG00000272765", "ENSG00000277365", "ENSG00000281013", "ENSG00000273397")
gene_info_new[gene_info_new$ensembl_gene_id %in% ndufa6,]

ptrpd <- c("ENSG00000153707", "ENSG00000282932")
gene_info_new[gene_info_new$ensembl_gene_id %in% ptrpd,]

pvalb <- c("ENSG00000100362", "ENSG00000274665")
gene_info_new[gene_info_new$ensembl_gene_id %in% pvalb,]

ruvbl1 <- c("ENSG00000175792", "ENSG00000284901")
gene_info_new[gene_info_new$ensembl_gene_id %in% ruvbl1,]
```

## TER missing ADI targets

```{r}
adis <- ter_source[ter_source$is_adi == TRUE,]
adis

adi_ensgs <- adis$ensembl_gene_id

target_ensgs <- target_list_source$ensembl_gene_id

setdiff(adi_ensgs, target_ensgs)
```

## new nominations '23 intial validation

```{r}
setdiff(gene_info_new$ensembl_gene_id, gene_info_old$ensembl_gene_id)
gene_info_new[gene_info_new$ensembl_gene_id == " ENSG00000166949",]
gene_info_new[gene_info_new$ensembl_gene_id == "ENSG00000166949",]

```
Are the new empty hgnc/name values due to a biomart refresh?
-> 110 in both, so no change in ensembl release?
```{r}
new_plec <- gene_info_new[gene_info_new$ensembl_gene_id == "ENSG00000178209",]
old_plec <- gene_info_old[gene_info_old$ensembl_gene_id == "ENSG00000178209",]

new_plec$ensembl_info
old_plec$ensembl_info
```

Why do we have +874 ensgs when we expected to have fewer due to igap filtering?
all exist in ensembl release 110
```{r}
new <- gene_info_new$ensembl_gene_id
old <- gene_info_old$ensembl_gene_id

new_ensgs <- setdiff(new, old)
removed_ensgs <- setdiff(old, new)

# what got removed?
for(ensg in removed_ensgs) {
  row <- gene_info_old[gene_info_old$ensembl_gene_id == ensg,]
  print(row$ensembl_gene_id)
}

# what got added?
for(ensg in new_ensgs) {
  row <- gene_info_new[gene_info_new$ensembl_gene_id == ensg,]
  ensembl_info <- data.frame(row$ensembl_info[1])
  print(row$ensembl_gene_id)
  print(ensembl_info$ensembl_release)
}
```
Spot check missing ENSGs
- checked 4x, all are X vs Y chr. - i guess this is ok? is this something new in ensembl release 110??

```{r}
# ASMTL-AS1
missing <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000292331',] # Y chr
agora <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000236017',] # X chr

missing$ensembl_info
agora$ensembl_info

# MIR6089
missing <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000277120',] # X chr
agora <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000292346',] # Y chr
...
```{r}
nominated_targets_new <- get_subobject(gene_info_new, "target_nominations")
mayo <- nominated_targets_new[grep("Mayo", nominated_targets_new$team),]
unique(mayo$initial_nomination)
```

do we have a 6-nom target yet? -> NOPE!
```{r}
unique(gene_info_new$total_nominations)
...
New nomination ENSG disambiguation (ASU multiple ENSG rows)
DCHS2
```{r}
row <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000197410',]
row
row$ensembl_info

row2 <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000284227',]
row2
row2$ensembl_info
```
...
TRPM1
```{r}
row <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000134160',]
row
row$ensembl_info

row2 <- gene_info_new[gene_info_new$ensembl_gene_id == 'ENSG00000274965',]
row2
row2$ensembl_info
```